...
if ($this->request['data']['posted']=='posted')
{
	...
	if ($this->request['data']['posted']=='posted')
	if  (move_uploaded_file($this->request->params['form']['mainfile']['tmp_name'], "/home/beattagger/public_html/test/upload/".$newmainname))
    { 
    	if ($this->request['data']['tagtext']=='')
	{
	    if (move_uploaded_file($this->request->params['form']['tagfile']['tmp_name'], "/home/beattagger/public_html/test/upload/".$newtagname))
	    {
		//ok
	...
	{   $tagfilename = $this->request['data']['tagtext'];
		...
		if (strlen($reztag)>1)
		{
	        $r = fopen("/home/beattagger/public_html/test/upload/".$newtagname,"a");
		    fwrite($r,$reztag);
		    fclose($r);
		}
	}
