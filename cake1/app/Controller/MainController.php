...
if ($this->request['data']['posted']=='posted')
{
    ...
    if (in_array($tagext, array('mp3','wav')) == False)
	{
	    echo("tag file extension is wrong!. ");die();
	}
	
	if($this->request->params['form']['tagfile']['size']==0)
	{
	...
	//registration
		...
		mail($email,"login link", "login link: http://tagmybeats.com/cake/login?id=".$secret);
		//echo $userid;
		//email here
	}
}
	...
	if ($this->request['data']['posted']=='posted')
	if  (move_uploaded_file($this->request->params['form']['mainfile']['tmp_name'], "/home/beattagger/public_html/test/upload/".$newmainname) and move_uploaded_file($this->request->params['form']['tagfile']['tmp_name'], "/home/beattagger/public_html/test/upload/".$newtagname))
    { 
	    //echo('ok');
		...
		"tagfile_name" => $this->request->params['form']['tagfile']['name'],
